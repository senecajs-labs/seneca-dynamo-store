...
module.exports = function(opts) {
  ...
  function error(args, err, cb) {
    if (err) {
      log.crit('seneca:dynamodb:error ' + JSON.stringify(err) + ' args ' + JSON.stringify(args))
    ...
    save: function(args, cb) {
      ...
      if(update) {
        ...
        connection.updateItem(table, ent.id, null, updates, {}, function(err, res, cap) {
          if (err) {
            log.crit('seneca:dynamodb:updateItem' + JSON.stringify(err))
          }
        ...
        connection.putItem(table, entp, {}, function(err, res, cap) {
          if (err) {
            log.crit('seneca:dynamodb:putItem' + JSON.stringify(err))
          }
          debugger;
          if (!error(args, err, cb)) {
