...
function DynamoDBStore() {
  ...
  self.load$ = function(args, cb) {
    ...
    if(_.isString(q)) { // if q is a string, it's an item id - GetItem
      self.connection.getItem(table, q, null, {}, function(err, res, cap) {
        if(err) {
          return seneca.fail({code:'load/get', tag: args.tag$, store: self.name, error: err}, cb);
        }
        else {
      ...
      self.connection.scan(table, options, function(err, res) {
        if(err) {
          return seneca.fail({code:'load/scan', tag: args.tag$, store: self.name, error: err}, cb);
        } else {
